...
// Funzione che implementa un semplice classificatore Random Forest con 2 alberi decisionali
int simple_classifier(float current_temp) {
    // Primo albero decisionale
    int tree1 = 0;  // OK by default
    if (current_temp < 20.0) {
        tree1 = 1;  // OUT_OF_RANGE
    } else if (current_temp > 30.0) {
        tree1 = 1;  // OUT_OF_RANGE
    }

    // Secondo albero decisionale
    int tree2 = 0;  // OK by default
    if (current_temp < 18.0) {
        tree2 = 1;  // OUT_OF_RANGE
    } else if (current_temp > 32.0) {
        tree2 = 1;  // OUT_OF_RANGE
    }

    // Media dei risultati dei due alberi (voto a maggioranza)
    int result = (tree1 + tree2) / 2;  // Se entrambi i risultati sono OK (0), ritorna OK. Altrimenti, OUT_OF_RANGE.
    return result;
}
