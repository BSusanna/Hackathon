...
void setup() {
  ...
  Serial.println("Sensor correctly initialized.");
}
...
void loop() {
  Wire.requestFrom(0x29, sizeof(temperature_data));  
  if (Wire.available()) {
    Wire.readBytes((byte*)&temperature_data, sizeof(temperature_data));
    Serial.print("Temperature received: ");
    Serial.println(temperature_data);

    int result = random_forest_classifier(temperature_data, n_trees);  

    if (result == 1) {
      Serial.println("Temperatura out of range.");
    } else {
      Serial.println("Temperatura within the range.");
    }
  }
...
#include <time.h>    // time(), simplify random generation
...
int random_forest_classifier(float current_temp, int n_trees) {
    ...
    for (int i = 0; i < n_trees; i++) {
        ...
        if (current_temp < 20.0 + (rand() % 5)) {  // change threshold
            tree_result = 1;
        } else if (current_temp > 30.0 + (rand() % 5)) {  // change threshold
            tree_result = 1;
        }

        // tree 2
        if (current_temp < 18.0 + (rand() % 5)) {
            tree_result = 1;
        } else if (current_temp > 32.0 + (rand() % 5)) { 
            tree_result = 1;
        }
    ...
    int result = (votes > n_trees / 2) ? 1 : 0;  // if the majority is out of range we return out of range (1)
    return result;
}
