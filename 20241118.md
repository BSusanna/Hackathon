...
float temperature_data = 0.0f;  // Dato della temperatura letto dalla Nicla
const int n_trees = 10;  // Numero di alberi per la Random Forest
...
void loop() {
  ...
  if (Wire.available()) {
    ...
    // Passa la temperatura al classificatore Random Forest
    int result = random_forest_classifier(temperature_data, n_trees);  // Passiamo il numero di alberi
...
#include <time.h>    // Per time(), usato per semplificare la generazione casuale

// Funzione che implementa un semplice classificatore Random Forest con n alberi decisionali
int random_forest_classifier(float current_temp, int n_trees) {
    srand(time(NULL));  // Inizializza il generatore di numeri casuali

    int votes = 0;  // Contatore per il voto a maggioranza

    for (int i = 0; i < n_trees; i++) {
        // Sottocampionamento casuale delle caratteristiche (in questo caso, solo 'current_temp')
        // In una vera Random Forest, avresti più caratteristiche da considerare per ogni albero

        // Simulazione dell'albero 1 (prima condizione casuale)
        int tree_result = 0;  // 0 = OK, 1 = OUT_OF_RANGE
        if (current_temp < 20.0 + (rand() % 5)) {  // Variamo la soglia casualmente
            tree_result = 1;
        } else if (current_temp > 30.0 + (rand() % 5)) {  // Variamo la soglia casualmente
            tree_result = 1;
        }

        // Simulazione dell'albero 2 (seconda condizione casuale)
        if (current_temp < 18.0 + (rand() % 5)) {  // Variamo la soglia casualmente
            tree_result = 1;
        } else if (current_temp > 32.0 + (rand() % 5)) {  // Variamo la soglia casualmente
            tree_result = 1;
        }

        // Sommiamo il risultato di questo albero
        votes += tree_result;
    }

    // Voto a maggioranza
    int result = (votes > n_trees / 2) ? 1 : 0;  // Se la maggioranza è OUT_OF_RANGE, ritorniamo OUT_OF_RANGE (1)
    return result;
}
